...
BATCH        = 128
REPLAY_MB    = 128
REPLAY_CAP   = 4096            # 20 exemplars × 10 classes
D_CLS, D_TSK = 128, 10
LR           = 1e-3
EMA_ALPHA    = 0.99
LAM_OT       = 2.0
...
print(device)
isMNIST, PERMUTED, ROT = True, False, True
# ────────────────────────────────────────────────
...
class RotatedMNIST(Dataset):
    def __init__(self, base, angle_deg):
        self.base = base
        self.angle = angle_deg
    def __getitem__(self, i):
        x, y = self.base[i]
        x = TF.rotate(x, self.angle, interpolation=TF.InterpolationMode.BILINEAR)
        return x.view(-1), y
...
def get_tasks():
    ...
    if ROT:
        perms=np.random.uniform(0, 2 * np.pi, size=5)
        return [DataLoader(RotatedMNIST(base,p),batch_size=BATCH,shuffle=True,
